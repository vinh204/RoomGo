...
public final class AuthRepository {
    ...
    public AuthRepository(UserDao userDao) { this.userDao = userDao; }
    ...
    public OperationResult<AuthData> register(String email, String phone, String password, String fullName) {
        try {
            ...
            User user = new User(0, normalized, phone, PasswordHasher.hash(password), fullName.trim(), System.currentTimeMillis());
            long id = userDao.insertUser(user);
            User saved = user.withId(id);
            return OperationResult.success(new AuthData(saved, String.valueOf(id)));
        } catch (Exception error) { return OperationResult.failure(error); }
    }

    public User getUserByEmail(String email) { return userDao.getUserByEmail(normalizeEmail(email)); }
    public User getUserByPhone(String phone) { return userDao.getUserByPhone(phone); }
    public OperationResult<String> getMongoUserIdByEmail(String email) {
        User user = getUserByEmail(email);
        return user == null ? OperationResult.failure(new Exception("Không tìm thấy tài khoản"))
            : OperationResult.success(String.valueOf(user.getId()));
    }
    public OperationResult<User> updateUser(long localUserId, String mongoUserId, String fullName, String newPassword) {
        ...
        String password = newPassword != null && !newPassword.trim().isEmpty() ? PasswordHasher.hash(newPassword) : current.getPassword();
    ...
    public OperationResult<User> syncUserFromBackend(long localUserId, String mongoUserId) {
        ...
        return user == null ? OperationResult.failure(new Exception("Không tìm thấy tài khoản")) : OperationResult.success(user);
    }
    private static String normalizeEmail(String email) { return email.toLowerCase(Locale.ROOT).trim(); }

    public static final class AuthData {
        private final User user; private final String mongoUserId;
        public AuthData(User user, String mongoUserId) { this.user = user; this.mongoUserId = mongoUserId; }
        public User getUser() { return user; } public String getMongoUserId() { return mongoUserId; }
    }
    ...
    public static final class LoginException extends Exception {
        ...
        public LoginException(String message, LocalLoginInfo authResponse) { super(message); this.authResponse = authResponse; }
        public LocalLoginInfo getAuthResponse() { return authResponse; }
    }
}
