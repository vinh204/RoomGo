...
public class BookingAdapter extends RecyclerView.Adapter<BookingAdapter.Holder> {
  public interface Listener {
    void onBooking(BookingWithRoom value);
  }

  private final Listener click;
  private List<BookingWithRoom> items = Collections.emptyList();

  public BookingAdapter(Listener c) {
    click = c;
  }

  public void submitList(List<BookingWithRoom> v) {
    items = v;
  ...
  public class Holder extends RecyclerView.ViewHolder {
    final TextView id, status, roomName, dates, guests, price;

    Holder(View v) {
      super(v);
      id = v.findViewById(R.id.tv_booking_id);
      status = v.findViewById(R.id.tv_status);
      roomName = v.findViewById(R.id.tv_room_name);
      dates = v.findViewById(R.id.tv_dates);
      guests = v.findViewById(R.id.tv_guests);
    ...
    void bind(BookingWithRoom value) {
      ...
      DateFormat df = new SimpleDateFormat("dd/MM", new Locale("vi", "VN"));
      dates.setText(
          "Nhận phòng: "
              + df.format(new Date(b.getCheckInDate()))
              + " · Trả phòng: "
              + df.format(new Date(b.getCheckOutDate())));
      guests.setText(b.getGuestCount() + " người lớn · 1 phòng");
      price.setText(
          NumberFormat.getNumberInstance(new Locale("vi", "VN")).format((long) b.getTotalPrice())
              + " đ");
      itemView.setOnClickListener(v -> click.onBooking(value));
    }
  }
