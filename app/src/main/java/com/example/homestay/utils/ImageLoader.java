...
public final class ImageLoader {
    ...
    public static void load(ImageView view, String source, int placeholder) {
        ...
        if ("content".equals(scheme) || "android.resource".equals(scheme) || "file".equals(scheme)) {
            try { view.setImageURI(uri); } catch (Exception ignored) { view.setImageResource(placeholder); }
            return;
        }
        ...
        EXECUTOR.execute(() -> {
            ...
            if (image != null) image.post(() -> { if (result != null) image.setImageBitmap(result); else image.setImageResource(placeholder); });
        });
    }
}
