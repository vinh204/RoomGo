...
public final class InputValidator {
  ...
  public static PasswordValidation validatePassword(String password) {
    ...
    if (!(lower.contains("password")
        ...
        || lower.contains("user"))) strength++;
    PasswordStrength result =
        strength <= 3
            ? PasswordStrength.WEAK
            : strength <= 4
                ? PasswordStrength.MEDIUM
                : strength <= 5 ? PasswordStrength.STRONG : PasswordStrength.VERY_STRONG;
  ...
  public static boolean validateEmail(String email) {
    if (email.isEmpty()
        || email.length() > 254
        || email.contains(" ")
        || !Patterns.EMAIL_ADDRESS.matcher(email).matches()) return false;
  ...
  public static boolean validatePhoneNumber(String phone) {
    ...
    return cleaned.matches("^0[0-9]{9}$")
        || cleaned.matches("^0[0-9]{10}$")
        || cleaned.matches("^\\+84[0-9]{9,10}$");
  }
  ...
  public static boolean validateFullName(String name) {
    return name.length() >= 2
        && name.length() <= 50
        && Pattern.compile("^[\\p{L}\\s'\\-]+$").matcher(name.trim()).matches();
  }
  ...
  public static final class PasswordValidation {
    ...
    public PasswordValidation(boolean valid, PasswordStrength strength) {
      this.valid = valid;
